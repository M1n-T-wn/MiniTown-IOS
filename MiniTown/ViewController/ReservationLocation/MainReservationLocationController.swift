...
class MainReservationLocationController: UIViewController, StompClientLibDelegate {
    ...
    func registerSocket() {
        ...
        print("==-=-=-=-\(wsURL)")
        let completedWSURL = "http://\(wsURL)/websoket"
        print("=-=-=-=-=-=-=-=-=-=-=-1")
    ...
    @objc func stompClientDidConnect(client: StompClientLib!) {
    ...
    @objc func stompClientDidDisconnect(client: StompClientLib!) {
        print("Socket is Disconnected")
        connectionLabel.text = "Socket is disconnected"
        connectionLabel.textColor = UIColor.purple
    }
    
    @objc func stompClient(client: StompClientLib!, didReceiveMessageWithJSONBody jsonBody: AnyObject?, akaStringBody stringBody: String?, withHeader header: [String : String]?, withDestination destination: String) {
        print("=-=-=-=-=-=-zxcvb")
    ...
    @objc func serverDidSendReceipt(client: StompClientLib!, withReceiptId receiptId: String) {
        print("=-=-=-=-=-=-dfghjk")

        print("Receipt : \(receiptId)")
    }
    
    @objc func serverDidSendError(client: StompClientLib!, withErrorMessage description: String, detailedErrorMessage message: String?) {
        print("=-=-=-=-=-=-yuil")
